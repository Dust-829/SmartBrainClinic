...
更新时间：2026-07-10。本文中的“31 张表”仍是 2026-07-09 数据库实例快照；AI 会话迁移状态以第 6 节验收清单为准。
...
当需要启用 AI 分诊会话、挂号绑定和 Medical 草稿上下文时，必须同时确认：

1. `20260708_01_create_ai_conversation_tables.sql` 已在目标数据库成功执行。
2. `ai_conversation_session`、`ai_conversation_message` 两张表实际存在，且应用账号具备读写权限。
3. 挂号创建时能够把有效 `triage_session_uuid` 绑定到 `register_uuid`。
4. `GET /api/v1/patient/register/{register_uuid}/ai-context` 能返回脱敏后的分诊上下文，缺失会话时返回明确的 404，而不是伪造数据。

以上是本地待合并 AI 上下文链路的运行前置条件；迁移未验收前，报告中只能描述为“已完成代码设计与本地实现”，不能描述为生产可用。
