...
└─ 右侧控制与读数：
   ├─ 阈值预设 0.30 / 0.50 / 0.70 + 可输入精确值（0.05–0.95）
   ├─ 掩码显示开关与透明度滑条（20%–90%）
   ├─ 当前层掩码像素数、总掩码像素数和体数据尺寸
   └─ 与检查报告编辑区保持分隔；不把像素数自动写入报告
```

视觉继续使用接诊页已有的深青绿主操作、浅蓝灰面板和紧凑圆角控件。主视图使用深色中性背景以保证 CT 灰阶层次；阈值预设为分段控件，当前值使用深青绿填充，不以红色表达风险。桌面端为左侧主视图、右侧控制栏；窄屏时改为主视图在上、控制栏在下，切片滑条和层切换按钮保持易点击尺寸。
...
3. 将成功任务区域改为查看器布局：主图、层导航、预设阈值、精确输入、透明度控制和只读元数据。阈值变化和层变化使用已防抖的图片 URL 更新；键盘左右方向键仅在滑条聚焦时切换切片。
...
3. 前端测试：预览图成功加载、三档阈值与精确阈值改变请求参数、切片滑条正确限幅、加载/失败/历史任务状态可读。
4. 人工验收：用当前 256 层 CT 任务验证首层、中间层、末层切换及 0.30/0.50/0.70 阈值差异；不把任何像素统计自动带入检查报告。
