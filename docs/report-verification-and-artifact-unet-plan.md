...
实施状态：进行中。单平面查看器已完成真实渲染、Gateway 转发、前端防抖和人工验收；三平面查看已完成真实渲染、独立空间定位和 voxel spacing 显示校正。当前待最后一次医生端人工交互验收后，再决定是否进入窗宽/窗位显示调节评估。
...
#### 原始 BrainCT 图形界面复盘与对齐计划（2026-07-14）

已核对 `D:\work\NeuEduPython\src\BrainCT` 及相邻的 `D:\work\NeuEduPython\src\Detection\CTArtGui.py`。后者才是原始桌面端查看器：它能加载 DICOM/MHD/NIfTI、异步执行 attention UNet2D 推理、显示轴位/冠状位/矢状位联动切片、以 VTK 展示 3D 表面，并将切片或参数变化以 300ms 防抖刷新。

| 原桌面端能力 | 医生端采用方式 | 本阶段结论 |
| --- | --- | --- |
| attention UNet2D 自动分割 | 复用，继续由独立 8013 模型服务执行 | 保留 |
| 轴位、冠状位、矢状位联动 | 后端按指定平面渲染受控 PNG，三张图共享同一空间定位 | 下一实施切片 |
| 300ms 防抖、异步进度 | 前端只在停止拖动后请求，显示局部加载状态；不重复推理 | 下一实施切片 |
| 掩码叠加与透明度 | 以保存的概率图实时阈值化、叠加，不伪造结果 | 当前切片必须完成 |
| 灰阶范围与视图窗 | 提供受限的窗宽/窗位或预设显示范围，不改变推理输入 | 多平面完成后评估 |
| VTK 3D 骨表面与掩码表面 | 不进入当前接诊页；浏览器端成本与临床工作台价值不匹配 | 暂不实现 |
| 灰度阈值、梯度、形态学、连通域规则分割 | 不与 UNet 推理结果混用，避免医生误以为它们属于模型输出 | 不实现 |
| 本地文件选择、掩码另存 | 生产链路由受控任务输入与输出目录管理 | 不实现 |

#### 调整后的实施顺序

1. **已完成：稳定单平面查看器**：Gateway 二进制图片代理、当前轴位切片、自由阈值、数值输入、透明度、掩码开关、约 300ms 防抖、加载与失败状态已完成，并经人工验收。
2. **代码已完成，待真实验收：补多平面浏览**：模型服务接收 `plane=axial|coronal|sagittal` 与统一体素位置；Medical 只代理渲染结果；前端提供三视图和同步定位，不向浏览器传 CT/NIfTI 体数据。三条独立滑杆分别控制轴位 Z、冠状位 Y、矢状位 X；三者共同定义一个可解释的空间位置，不再把一个层号按比例映射到另外两个方向。

性能处理：模型服务以任务、来源和概率图引用为键缓存最近两组已完成的 CT 体数据与概率图，避免重复读取 DICOM/NIfTI；前端拖动某一方向的位置滑杆时只刷新该方向图像，只有阈值、透明度和掩码开关会刷新三张图。

本地技术验收（2026-07-15）：对同一已完成任务的轴位 PNG 连续请求，首次为约 136ms，缓存命中后约 65ms；浏览器日志同时确认位置滑杆仅请求对应平面。
3. **最后评估显示调节**：只在多平面稳定后决定是否增加受限窗宽/窗位。它仅改变展示映射，不能改变模型概率或报告内容。
4. **保持报告边界**：所有查看器控件仅影响当前医生可见的图像；检查报告仍由医生独立填写、审核和发布。

#### 设计图决策

现有设计图对第一步“单平面交互式查看器”仍然有效。多平面浏览采用下图作为已确认的接诊页内嵌三视图方向：轴位为主视图，冠状位与矢状位并列；右侧统一保留切片定位、阈值、透明度和掩码显示控制。不会沿用桌面端 VTK 的双栏布局，也不会新增独立页面。

![接诊页内嵌三平面 CT 伪影查看器设计方向图](./assets/doctor-encounter-ct-triplanar-concept-2026-07-15.png)

草图仅确认层级、布局、配色与同步交互，不表示引入肺部影像、床位信息、住院工作台导航或图中示例数值；实际实现仍限定为本项目的头颅 CT、当前检查条目与受控后端渲染。
