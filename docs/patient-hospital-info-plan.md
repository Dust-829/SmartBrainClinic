...
- 首页 `PatientHomeView.vue` 的“医院信息”功能格已跳转至 `/patient/hospital`。
- `frontend/src/router/index.ts` 已注册公开路由 `/patient/hospital`，`PatientLayout.vue` 已将其纳入沉浸式页面列表。
...
1. 已新增 `PatientHospitalInfoView.vue`，注册为公开路由 `/patient/hospital`，名称为 `patient-hospital`。
2. 已将首页“医院信息”功能格跳转至该路由。
3. 已将路由加入 `PatientLayout.vue` 的沉浸式页面列表，复用患者端移动页面壳与底部导航。
4. 返回操作固定回 `/patient/home`，避免在没有历史记录的新窗口中回退到空白页。

### 4.2 内容与交互

- 已在页面内提供概念文案和“概念展示”提示；未创建接口、store 或 mock API。
- “医院简介”“神经专科门诊”“就诊指南”作为页内可读内容展示，不制造尚未实现的独立业务页面。
- “路线与地图”点击显示“地图导航服务暂未接入，请提前规划来院路线。”，不伪造定位、地图或第三方跳转成功。
- “查看就诊指南”会平滑滚动并将焦点置于指南区块；若后续拆分真实指南页，再替换为路由跳转。
...
- 路由：已从 `/patient/home` 的“医院信息”进入 `/patient/hospital`，并验证返回后落在首页。
- 视觉：已在 390px、430px 宽度下检查无横向溢出；桌面预览继续由 `PatientLayout` 统一包裹。
- 内容：概念数据不冒充真实接口数据，未接入能力有明确提示。
- 交互：已验证指南定位与焦点移动、路线提示、底部导航一致性。
- 构建：`npm run build` 已通过；浏览器控制台无错误。
