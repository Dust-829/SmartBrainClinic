...
2. **已完成（2026-07-13）**：新增 `/patient/payments` 受保护路由、患者端缴费 API 类型与 `PatientPaymentCenterView.vue`；首页宫格已将“按科室挂号”替换为“缴费中心”。页面支持加载、空态、失败、同挂号限定选择、微信/支付宝模拟支付与支付后的三次状态刷新。
