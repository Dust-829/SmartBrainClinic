...
- “缴费记录”已通过患者侧 BFF 聚合本人挂号下的账单，并提供受登录保护的 `/patient/payment-records` 页面；空态与页头操作均指向该真实数据页。
- 有待缴项目时沿用同一内容卡展示按挂号分组的清单和底部结算栏，避免空态与有数据状态发生视觉断层。

该示意图已落实到 `PatientPaymentCenterView.vue`：页头下的状态卡、统一内容卡、紧凑空态和“查看缴费记录”操作均已完成。
...
- `GET /api/v1/patient/{patient_uuid}/payment-records`
  - Patient 服务仅取得本人挂号，再按挂号调用 Billing 的既有账单查询；返回账单号、金额、支付方式、支付时间和账单状态。
  - 已支付与已退费账单均保留在记录中，前端使用账单状态区分展示，避免将退费历史静默隐藏。
...
4. **已完成（2026-07-13）**：按上述示意图重构 `/patient/payments` 的页头下状态区、空态和内容卡；空态操作改为“查看缴费记录”。新增 `GET /api/v1/patient/{patient_uuid}/payment-records`、受保护的 `/patient/payment-records` 路由与账单列表，入口已指向真实患者账单数据。
