...
- 已完成 AI 检查检验建议的受控规则候选、只读接口和接诊页操作闭环，详见 [AI 检查检验建议实施计划](./ai-check-inspection-recommendation-plan.md)。候选仅基于病历、AI 分诊上下文和已开项目生成；医生可忽略候选，或编辑检查项目的部位和目的后加入待签清单，最终仍复用统一签署动作。浏览器回归已断言候选加入清单本身不创建真实订单。
...
1. 按 [AI 检查检验建议实施计划](./ai-check-inspection-recommendation-plan.md) 接入可选 LLM 排序和理由生成；白名单校验已完成，后续仅接入现有模型调用与脱敏审计，且不自动创建订单。
2. 非空医生队列可视化回归。
3. 管理员真实鉴权和排班管理。
4. 三端完整演示回归。
