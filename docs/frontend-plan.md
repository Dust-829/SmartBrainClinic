...
| `/patient/queue` | 兼容旧链接，重定向至“挂号与候诊” |
| `/patient/registers` | 挂号与候诊，需登录 |
...
- `/patient/registers` 是“挂号与候诊”的唯一患者端入口：展示全部挂号记录，并为“已挂号”和“接诊中”记录在列表内展开候诊详情；每 15 秒自动同步候诊进度，保留手动刷新、加载失败提示与诊室信息。
- `/patient/queue` 仅保留为兼容旧链接的重定向，会携带挂号单参数回到 `/patient/registers`；支付完成后也直接打开对应挂号单的候诊详情。已结束、已退号和待支付记录不展示候诊入口。
