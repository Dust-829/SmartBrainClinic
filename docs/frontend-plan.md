...
- 医生端以桌面工作台为验收目标；现有响应式样式可以保留，但不再作为窄屏产品需求或回归标准。
...
- 接诊页左侧固定为医生工作区，右侧集中展示 AI 分诊、AI 处方建议、相似病历和 AI 医生助手；当前以桌面并列布局为准。
...
- 医生端不再以窄屏布局作为验收目标；后续浏览器回归只覆盖桌面工作台与接诊主链。

### 已完成回归：医生端桌面浏览器脚本

交付标准：

- 已在 `frontend/` 引入 Playwright 和 Chromium，`pnpm test:e2e` 已覆盖医生登录页只读冒烟和桌面端处方完整写操作；失败时保留 trace 和截图。
...
- 脚本必须使用受控演示数据或可恢复数据，避免重复开立正式处方；桌面主链保留断言或截图证据。
