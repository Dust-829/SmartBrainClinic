...
- AI 处方建议仅在病历确认后生成，医生可调整用法、数量或移除药品，再明确确认开立；AI 检查检验建议在病历确认后按需生成，医生可编辑检查部位和目的、忽略候选或加入待签清单，但只有统一签署才会创建订单。
...
- 已完成 AI 检查检验建议闭环，详见 [AI 检查检验建议实施计划](./ai-check-inspection-recommendation-plan.md)。候选基于病历、AI 分诊上下文和已开项目生成；规则候选非空时可由 LLM 在白名单内排序并补充理由，失败自动降级。医生可忽略候选，或编辑检查项目的部位和目的后加入待签清单，最终仍复用统一签署动作。浏览器回归已断言候选加入清单本身不创建真实订单，真实服务回归已验证 LLM 无效载荷会在约 8 秒内记录审计并降级。
...
- 已在 `frontend/` 引入 Playwright 和 Chromium，`pnpm test:e2e` 已覆盖医生登录页只读冒烟、处方完整写操作和窄屏布局；失败时保留 trace 和截图。
...
- 已通过 Playwright 处方主链回归：测试先运行受控造数脚本，再以生成医生身份登录、打开对应接诊页、填写必填病历字段并确认；已生成真实处方建议、确认至少一项候选药品、将第一项数量调整为 `2`，并点击正式开立。测试随后按处方 UUID 回查药房详情，确认关联挂号、AI 推荐标识和数量均正确。
- 已通过 `390 x 844` 窄屏 Playwright 布局回归：使用独立受控接诊挂号，以实际渲染后的网格列数、医生工作区与 AI 支持区的纵向先后关系、页面横向滚动宽度作为断言；该检查不确认病历、不生成建议、不创建处方。
- 已将“登录 -> 候诊 -> 接诊 -> 病历确认 -> 生成建议 -> 调整数量 -> 处方确认”链路固化为可重复执行的浏览器回归脚本。
- 脚本必须使用受控演示数据或可恢复数据，避免重复开立正式处方；桌面和窄屏均保留断言或截图证据。当前桌面完整开立链和 `390 x 844` 窄屏布局断言均已固化。

### 后续切片

当前阶段总结：医生接诊主链、AI 处方建议和 AI 检查检验建议已完成受控实现与浏览器回归。检查检验候选只会进入待签清单，不会自动开单；真实 LLM 无效或超时会在约 8 秒内降级并写入脱敏审计。下一步优先补医生工作台的真实非空队列回归。
