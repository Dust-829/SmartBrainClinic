...
- 已完成 AI 检查检验建议的接口、安全边界和分步实施设计，详见 [AI 检查检验建议实施计划](./ai-check-inspection-recommendation-plan.md)。后续仍仅基于病历、AI 分诊上下文和已开项目给出候选项目与理由；医生选择、删除或调整后，复用统一开单待签清单和确认签署动作。
...
1. 按 [AI 检查检验建议实施计划](./ai-check-inspection-recommendation-plan.md) 在接诊页接入候选卡：只读接口已返回已确认病历、真实目录和已开项目过滤后的检查、检验候选及理由；不自动创建订单。
2. 非空医生队列可视化回归。
3. 管理员真实鉴权和排班管理。
4. 三端完整演示回归。
