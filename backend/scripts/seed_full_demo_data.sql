...
SELECT r.rule_uuid, e.employee_uuid, r.rule_name, r.week_rule, r.llm_text_rule, r.regist_quota, room.room_uuid, 1 FROM tmp_demo_schedule_rules r JOIN tmp_demo_employees e ON e.employee_key=r.doctor_key JOIN tmp_demo_rooms room ON room.room_key=r.room_key
ON CONFLICT (uuid) DO UPDATE SET employee_uuid=EXCLUDED.employee_uuid, rule_name=EXCLUDED.rule_name, week_rule=EXCLUDED.week_rule, llm_text_rule=EXCLUDED.llm_text_rule, regist_quota=EXCLUDED.regist_quota, clinic_room_uuid=EXCLUDED.clinic_room_uuid, delmark=1;
INSERT INTO scheduling_actual (uuid, employee_uuid, schedule_date, noon, regist_quota, registered_count, clinic_room_uuid)
SELECT s.schedule_uuid, e.employee_uuid, CURRENT_DATE+s.day_offset, s.noon, s.regist_quota, 0, room.room_uuid FROM tmp_demo_schedules s JOIN tmp_demo_employees e ON e.employee_key=s.doctor_key JOIN tmp_demo_rooms room ON room.room_key=s.room_key
ON CONFLICT (uuid) DO UPDATE SET employee_uuid=EXCLUDED.employee_uuid, schedule_date=EXCLUDED.schedule_date, noon=EXCLUDED.noon, regist_quota=EXCLUDED.regist_quota, clinic_room_uuid=EXCLUDED.clinic_room_uuid;
INSERT INTO scheduling_time_slot (uuid, scheduling_actual_id, time_range, is_booked)
SELECT pg_temp.sbc_demo_uuid('slot:'||s.schedule_key||':'||n), sa.id, CASE WHEN s.noon='上午' THEN '08:'||LPAD(((n-1)*10)::text,2,'0')||'-08:'||LPAD((n*10)::text,2,'0') ELSE '13:'||LPAD(((n-1)*10)::text,2,'0')||'-13:'||LPAD((n*10)::text,2,'0') END, FALSE FROM tmp_demo_schedules s JOIN scheduling_actual sa ON sa.uuid=s.schedule_uuid CROSS JOIN generate_series(1,4) n
