...
logger = logging.getLogger("patient.outbox_sweeper")
...
async def sweep_outbox_events():
    ...
    """
    logger.info("📤 [Outbox Sweeper] Started background task for Outbox Pattern.")
            ...
            async with session_factory() as session:
                ...
                for event in events:
                    try:
                        ...
                        logger.error(f"⚠️ [Patient Outbox Sweeper] Failed to publish event {event.uuid}: {pub_err}")
                        event.retry_count = (event.retry_count or 0) + 1
                        if event.retry_count >= 3:
                            event.status = "dead_letter"
                            logger.error(f"⚠️ [Patient Outbox Sweeper] Event {event.uuid} moved to dead_letter after 3 retries.")
                        session.add(event)
                        
                if events:
                    await session.commit()
                    logger.info(f"📤 [Outbox Sweeper] Successfully processed and sent {len(events)} events.")
                    
        except Exception as e:
            logger.error(f"⚠️ [Outbox Sweeper] Error during outbox sweep: {e}", exc_info=True)
